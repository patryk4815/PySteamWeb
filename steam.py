...
class SteamWebBase(object)
    ...
    def _init_session(self)
        ...
        session.headers.update({
            'User-Agent' 'Mozilla5.0 (Windows NT 6.3; Win64; x64) AppleWebKit537.36 (KHTML, like Gecko) Chrome45.0.2453.0 Safari537.36'
        })
        return session
    ...
    def _check_is_login(self)
        chat_html = self.send_session(
            url='httpsteamcommunity.comchat',
            is_post=False
        )
    ...
    def whitelist_cookie(self, name)
        whitelists = [
            r'^steamLogin$',
            r'^steamLoginSecure$',
            r'^steamMachineAuthd+$',  # quardian secret code
    ...
    def login(self, kwargs)
        query_data = {
            ...
            'captchagid' '-1',           # httpsstore.steampowered.comjoinrefreshcaptcha
            'captcha_text' '',         # httpsstore.steampowered.compubliccaptcha.phpgid=gid
        ...
        rsa_data = self.send_request(
            url='httpssteamcommunity.comlogingetrsakey',
        ...
        query_data['password'] = self.encrypt_password(query_data.get('password'), rsa_data.get('publickey_mod'), rsa_data.get('publickey_exp'))

        logging.info(self.session_get_cookies())
        login_data = self.send_session(
            url='httpssteamcommunity.comlogindologin',
            data=query_data,
            is_post=True,
            is_json=True
        )
        ...
        if not login_data.get('success', False)
            if login_data.get('emailauth_needed', False)
                ...
                kwargs['captcha_text'] = input('Podaj kod z obrazka httpsstore.steampowered.compubliccaptcha.phpgid={}  '.format(login_data['captcha_gid']))
